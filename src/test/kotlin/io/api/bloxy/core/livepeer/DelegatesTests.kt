 ...
 * @since 14.03.2019
 */
class DelegatesTests : BloxyTester() {

    @Test
    fun valid() {
        val list = api.livepeer.delegates()
        assertNotNull(list)
        ...
        mustValid(list[0].delegators)
        mustValid(list[0].firstTxAt)
        assertNotNull(list[0].lastTxAt)
        mustValid(list[0].rebonded)
        mustValid(list[0].unbonded)
    ...
    fun `address not exist`() {
        ...
        val list = api.livepeer.delegates(roundManagerProxy)
        assertNotNull(list)
