 ...
 * @since 01.03.2019
 */
class AddrTopSendersTests : BloxyTester() {

    @Test
    fun valid() {
        val address = "0xC0ea08A2d404d3172d2AdD29A45be56dA40e2949"
        val result = api.maltego.addrTopSenders(address, limit = 5)
    ...
    fun `valid with contract`() {
        ...
        val result = api.maltego.addrTopSenders(address, contract)
        assertNotNull(result)
        assertFalse(result.isEmpty())
        assertFalse(result[0].isEmpty())
    }
    ...
    fun `non exist address empty result`() {
        ...
        val result = api.maltego.addrTopSenders(address)
        assertNotNull(result)
        assertTrue(result.isEmpty())
    }
    ...
    fun `invalid address param error`() {
        ...
        api.maltego.addrTopSenders(address)
    }
}