 ...
 * @since 01.03.2019
 */
class AddrTopReceiversTests  : BloxyTester() {
    ...
    fun `valid with contract`() {
        ...
        val result = api.maltego.addrTopReceivers(address, contract)
        assertNotNull(result)
        assertFalse(result.isEmpty())
        assertFalse(result[0].isEmpty())
    }
    ...
    fun `non exist address empty result`() {
        ...
        val result = api.maltego.addrTopReceivers(address)
        assertNotNull(result)
        assertTrue(result.isEmpty())
    }
    ...
    fun `invalid address param error`() {
        ...
        api.maltego.addrTopReceivers(address)
    }
}