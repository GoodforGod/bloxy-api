 ...
 * @since 01.03.2019
 */
class AddrTransfersSentTests : BloxyTester() {

    @Test
    fun valid() {
        val addresses = listOf("0xC0ea08A2d404d3172d2AdD29A45be56dA40e2949")
        val result = api.maltego.addrTransfersSent(addresses, since = LocalDate.of(2016, 1, 1))
        ...
        assertNotNull(result[0].receiverFlag)
    ...
    fun `valid with contract`() {
        ...
        val result = api.maltego.addrTransfersSent(addresses, contracts, since = LocalDate.of(2016, 1, 1))
        assertNotNull(result)
        assertFalse(result.isEmpty())
        assertFalse(result[0].isEmpty())
    }
    ...
    fun `valid address non exist contract`() {
        ...
        val result = api.maltego.addrTransfersSent(addresses, contracts)
        assertNotNull(result)
    ...
    fun `non exist address empty result`() {
        ...
        val result = api.maltego.addrTransfersSent(addresses)
        assertNotNull(result)
        assertTrue(result.isEmpty())
    }
    ...
    fun `invalid address param error`() {
        ...
        api.maltego.addrTransfersSent(addresses)
    }
}