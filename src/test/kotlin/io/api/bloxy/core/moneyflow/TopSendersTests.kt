 ...
 * @since 25.11.2018
 */
class TopSendersTests : BloxyTester() {

    @Test
    fun valid() {
        val address = "0xC0ea08A2d404d3172d2AdD29A45be56dA40e2949"
        val result = api.moneyFlow.topSenders(address, limit = 5)
    ...
    fun `valid with contract`() {
        ...
        val result = api.moneyFlow.topSenders(address, contract)
        assertNotNull(result)
        assertFalse(result.isEmpty())
        assertFalse(result[0].isEmpty())
    }
    ...
    fun `invalid address param error`() {
        ...
        api.moneyFlow.topSenders(address)
    }
}